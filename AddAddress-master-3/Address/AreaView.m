...
- (void)creatBaseUI
{
    ...
    for (int i = 0; i < 4; i++) {
        UITableView *area_tableView = [[UITableView alloc]initWithFrame:CGRectMakes(375* MULPITLE  * i, 0, 375, 300) style:UITableViewStylePlain];
...
- (void)setCityArray:(NSMutableArray *)cityArray
{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        _areaScrollView.contentOffset = CGPointMake(375 * MULPITLE * 2, 0);
    }];
}
- (void)setRegionsArray:(NSMutableArray *)regionsArray
{
    ...
    [tableView reloadData];
    _areaScrollView.contentSize = CGSizeMake(375 * MULPITLE * 4, 300 * MULPITLE);
    [UIView animateWithDuration:0.5 animations:^{
        _areaScrollView.contentOffset = CGPointMake(375 * 3 * MULPITLE, 0);
    }];
}
...
-(void)setCountriesArray:(NSMutableArray *)countriesArray{
    ...
    self.indexView  = [[IndexView alloc]initWithFrame:CGRectMake((375* MULPITLE - 20), 0, 20, 380)];
    
    [self.indexView setBackgroundColor:[UIColor colorWithRed:(238/255.0) green:(238/255.0) blue:(238/255.0) alpha:1]];
    
    [self.indexView setDelegate:self];
    [self.indexView setDataSource:self];
    
    [self.indexView reloadItemViews];
    
    [_areaScrollView addSubview:self.indexView];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (tableView.tag - 200) {
        case 0:
        {
            ...
            NSArray *array = dic[@"subArray"];
            AddressAreaModel *addressAreaModel = array[indexPath.row];
...
//
            ...
            [btn4 setTitle:@"请选择" forState:UIControlStateNormal];
...
-(UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    UIView * titleView = [[UIView alloc] initWithFrame:CGRectMake(0,0, 375, 30)];
...
- (void)hidenAreaView
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        if ([self.address_delegate respondsToSelector:@selector(getSelectAddressInfor:)]) {
            [self.address_delegate getSelectAddressInfor:[NSString stringWithFormat:@"%@%@%@%@",btn1.titleLabel.text,btn2.titleLabel.text,btn3.titleLabel.text,btn4.titleLabel.text]];
        }
    }];
}
